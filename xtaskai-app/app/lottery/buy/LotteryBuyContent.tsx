...
export default function LotteryBuyContent() {
  ...
  if (status === "success") return <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4"><div className="text-center max-w-md w-full bg-white rounded-2xl shadow-lg p-8"><div className="text-6xl mb-4">✅</div><h2 className="text-2xl font-bold text-green-600 mb-2">Ticket Purchased!</h2><p className="text-gray-600">You joined the lottery.</p><p className="text-gray-400 text-sm mt-4">Redirecting...</p></div></div>;

  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-lg p-8">
        <div className="text-center"><div className="text-5xl mb-4">🎰</div><h1 className="text-2xl font-bold mb-2">Buy Lottery Ticket</h1><p className="text-gray-500 mb-6">Round #{roundId}</p></div>
        <div className="bg-gray-50 rounded-xl p-4 mb-6">
          <div className="flex justify-between mb-2"><span className="text-gray-500">Ticket Price:</span><span className="font-bold">0.01 USDC</span></div>
          <div className="flex justify-between mb-2"><span className="text-gray-500">Fee:</span><span className="font-bold text-orange-500">0.002 USDC</span></div>
          <div className="flex justify-between pt-2 border-t"><span className="font-semibold">Total:</span><span className="font-bold text-green-600">0.012 USDC</span></div>
          <div className="flex justify-between mt-2"><span className="text-gray-500">Wallet:</span><span className="font-mono text-sm">{walletAddress?.slice(0,8)}...{walletAddress?.slice(-6)}</span></div>
          <div className="flex justify-between mt-2"><span className="text-gray-500">Reward:</span><span className="font-bold text-purple-600">+100 XTP</span></div>
        </div>
        ...
        {needsApproval ? (
          ...
          <button onClick={handleBuyTicket} disabled={status==="depositing"} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-xl disabled:opacity-50">
            {status==="depositing"?<span className="flex items-center justify-center gap-2"><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"/>Processing...</span>:"Pay 0.012 USDC & Buy Ticket"}
          </button>
        )}
        <button onClick={()=>window.location.href="https://xtaskai.com/base-mini-app/lottery.php"} className="w-full mt-3 text-gray-500 text-sm py-2">Cancel</button>
      </div>
    </div>
  );
}
