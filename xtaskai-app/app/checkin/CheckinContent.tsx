...
export default function CheckinContent() {
  ...
  async function registerCheckin(txHash: string) {
    try {
      ...
      if (data.success) {
        setStatus("success");
        setTimeout(() => { window.location.href = "https://xtaskai.com/base-mini-app/quests.php?success=Check-in+successful!+100+XTP"; }, 2000);
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-lg p-8">
        ...
        <div className="bg-gray-50 rounded-xl p-4 mb-6 space-y-2">
          <div className="flex justify-between border-b border-gray-100 pb-2"><span className="text-gray-500">Fee:</span><span className="font-bold">0.001 USDC</span></div>
          <div className="flex justify-between border-b border-gray-100 pb-2"><span className="text-gray-500">Reward:</span><span className="font-bold text-purple-600">+100 XTP</span></div>
        ...
        {needsApproval ? (
          ...
          <button onClick={handleCheckin} disabled={status === "depositing"} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-xl disabled:opacity-50">
            {status === "depositing" ? <span className="flex items-center justify-center gap-2"><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"/>Processing...</span> : "Pay 0.001 USDC & Check-in"}
          </button>
        )}
