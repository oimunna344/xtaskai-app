...
export default function RedeemPayContent() {
  ...
  if (status === "success") return <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4"><div className="text-center max-w-md w-full bg-white rounded-2xl shadow-lg p-8"><div className="text-6xl mb-4">✅</div><h2 className="text-2xl font-bold text-green-600 mb-2">Reward Claimed!</h2><p className="text-gray-600">You received:{reward_usdc>0?` $${reward_usdc} USDC`:""}{reward_xtp>0?` ${reward_xtp} XTP`:""}</p><p className="text-gray-400 text-sm mt-4">Redirecting...</p></div></div>;
  if (!code_id || !code) return <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4"><div className="text-center max-w-md w-full bg-white rounded-2xl shadow-lg p-8"><div className="text-6xl mb-4">❌</div><h2 className="text-2xl font-bold text-red-600 mb-2">No Redeem Data</h2><p className="text-gray-600 mb-6">Please go back and enter your code.</p><button onClick={()=>window.location.href="https://xtaskai.com/base-mini-app/redeem.php"} className="w-full bg-blue-600 text-white py-3 rounded-xl">Go Back</button></div></div>;

  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-lg p-8">
        <div className="text-center"><div className="text-5xl mb-4">🎁</div><h1 className="text-2xl font-bold mb-2">Claim Reward</h1><p className="text-gray-500 mb-6">Pay fee to claim your reward</p></div>
        <div className="bg-gray-50 rounded-xl p-4 mb-6 space-y-2">
          {[["Code", code],["Reward", `${reward_usdc>0?`$${reward_usdc} USDC`:""} ${reward_xtp>0?`${reward_xtp} XTP`:""}`],["Claim Fee","0.005 USDC"],["Wallet",`${walletAddress?.slice(0,8)}...${walletAddress?.slice(-6)}`]].map(([k,v])=>(
        ...
        <div className="bg-blue-50 rounded-xl p-3 mb-4 text-sm text-blue-800">💡 Reward will be added to your XTaskAI balance!</div>
        ...
        {needsApproval ? (
          ...
          <button onClick={handlePayFee} disabled={status==="depositing"} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-xl disabled:opacity-50">
            {status==="depositing"?<span className="flex items-center justify-center gap-2"><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"/>Processing...</span>:"Pay 0.005 USDC & Claim Reward"}
          </button>
        )}
        <button onClick={()=>window.location.href="https://xtaskai.com/base-mini-app/redeem.php"} className="w-full mt-3 text-gray-500 text-sm py-2">Cancel</button>
      </div>
    </div>
  );
}
